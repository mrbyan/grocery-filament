<div class="pb-4">
    ...
    <div class="px-4 mt-4 w-full">
        <div class="rounded-sm h-60 overflow-hidden">
            <img src="{{ asset('images/home-banner.jpg') }}" alt="ad" class="opacity-90" />
        </div>
    </div>
        ...
        <div class="flex space-x-4 mt-4 overflow-x-scroll">
            @foreach ($categories as $category)
                <a href="{{ '/products-category/' . $category->slug }}" wire:navigate
                    wire:key="category-{{ $category->id }}" class="flex flex-col items-center gap-3 w-24">
                    <div class="w-16 h-16">
                        <img class="rounded-full bg-primary/10 p-4 w-full h-full object-cover"
    ...
    <div class="mt-4 px-4">
        ...
        <div class="grid grid-cols-2 gap-6 mt-4">
            @foreach ($products as $product)
                <div class="flex flex-col items-center rounded relative bg-white"
